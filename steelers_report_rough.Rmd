...
```
|
|-------------------**Plot this with logos**



**Chart** Do people ultimately end up in the stadium? 
...
Face values of originally purchased games + value of games sold on secondary markets + number of games ticket was forwarded / total amount of unique IDs
...
Mutually beneficial = people who buy tickets and come to a lot of games
...
```{r}
#Fans who went to at least one game  
...
#percentage that re-sold at least oe game 

#Only 2 customers went to more games than not 

#cancel candidates 
data
cancel <- filter(data, ticket_action =="Actvie"  | ticket_action == "Forward", attended =="N")
...
```{r}
#kpres <- kproto(datacopy,6, lambda = 25)
#see = clprofiles(kpres, datacopy)
#see
```
...
Target those people who are: 
- not forwarding their ticket, not attending (wasted game, low attendance).
- those who are forwarding, not attending (what can we do to get you to the game?)
- core fans, those who bought the ticket? get them in as season ticket holders opposed to one game purchasers on secondary market. (show price comparison)

4. Secondary lift (left?) over face value for each game
Game by game show the average increase in secondary market vs. first game
...
**Bar Graph** showing game by game 
